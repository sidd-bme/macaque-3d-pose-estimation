import src.third_party.anipose.filter_pose as af 
import numpy as np
import os
import pandas as pd
import glob 
...
import toml
import h5py
from src.third_party.aniposelib.cameras import CameraGroup
...
def proc(data_name,results_dir_root, config_path, n_kp, redo=False):
    ...
    with open(result_dir + '/kp2d.pickle', 'rb') as f:
        kp2d = pickle.load(f)
    ...
    for i_animal in range(n_animal):
        ...
        if config['triangulation']['optim']:
            ...
            np.save(result_dir + '/joint_len.npy', np.array(joint_len))
