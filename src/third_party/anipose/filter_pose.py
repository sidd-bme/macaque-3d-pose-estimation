...
def remove_dups(pts, thres=7): #changed to handle VITPOSE all NAN/0 values when missed detection
    ...
    #NEW^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    # prevent scipy KD-tree from crashing on NaN / Inf values
    pts_ix = np.where(np.isfinite(pts_ix), pts_ix, 1e9)
    tree = cKDTree(pts_ix.reshape(-1, 3))
    #NEW^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
